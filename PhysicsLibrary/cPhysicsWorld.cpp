	...
	void cPhysicsWorld::TimeStep(float deltaTime)
	{
		...
		for (int index = 0; index < mVecRigidBodies.size(); index++)
		{
			...
			if (pCurGO->mShape->getShapeType() == SHAPE_SPHERE)
			{
				...
				//glm::vec3 deltaVelocity = ((float)deltaTime * pCurGO->mAccel)
				//	+ ((float)deltaTime * GRAVITY);

				//pCurGO->mVelocity += deltaVelocity;

				//// New position is based on velocity over time
				//glm::vec3 deltaPosition = (float)deltaTime * pCurGO->mVelocity;
				//pCurGO->mPosition += deltaPosition;
